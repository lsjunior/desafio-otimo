export class RfbUtils {
  ...
  // https://medium.com/@guilherme_manzano/como-fazer-um-validador-de-cnpj-em-tyepscript-e-ionic-4-a06c0d1bba4c
  public static isValidCnpj(cnpj: string): boolean {
    ...
    // Os quatro blocos seguintes de funções irá reaizar a validação do CNPJ propriamente dito, conferindo se o DV bate.
    // Caso alguma das funções não consiga verificar o DV corretamente, mostrará uma mensagem de erro ao usuário e retornará falso,
    ...
    for (let k = tamanho; k >= 1; k--) {
      soma += parseInt(numeros.charAt(tamanho - k), 10) * pos--;
      if (pos < 2) {
        pos = 9;
      }
    }
